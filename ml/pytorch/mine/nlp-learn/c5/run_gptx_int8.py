...
parser = argparse.ArgumentParser('GPT-neox generation script', add_help=False)
parser.add_argument(
    '-m', '--model-id', default=None, type=str, required=True, help="your GPT-neox model"
)
parser.add_argument(
    "--device",
    type=str,
...
config = AutoConfig.from_pretrained(
    args.model_id, low_cpu_mem_usage=True, torchscript=args.jit
)
if not hasattr(config, "text_max_length") and args.prompt is None:
    config.text_max_length = int(args.input_tokens) + int(args.max_new_tokens)
user_model = AutoModelForCausalLM.from_pretrained(args.model_id, config=config)
